(() => {
  ...
  function addRowWithPractice(prac) {
    ...
    //se Agrega cantidad por defecto
    const inputCantidad = row.querySelector('.practica-cantidad');
    if (inputCantidad) {
        // Establecemos el valor a 1 por defecto (si la plantilla no lo hizo)
        inputCantidad.value = 1; 
    }
  }

  container.addEventListener('input', function (e) {
        const input = e.target.closest('.practica-cantidad');
        if (!input) return;
    
        const valor = parseInt(input.value);
        const max = parseInt(input.getAttribute('max')); // Lee el atributo max del input

        if (valor < 1) {
            input.setCustomValidity("La cantidad no puede ser menor a 1.");
        } else if (max && valor > max) {
            input.setCustomValidity(`La cantidad no puede superar ${max}.`);
        } else {
            input.setCustomValidity(""); // Limpiar el error si es válido
        }
        // Muestra el mensaje de error nativo del navegador (solo si es inválido)
